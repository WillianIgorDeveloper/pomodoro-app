...
export function LongBreakPage() {
  const context = useGlobalContext()

  const [minutes, setMinutes] = useState(context.longBreakPeriod.split(":")[0])
  const [seconds, setSeconds] = useState(context.longBreakPeriod.split(":")[1])

  function updateLocalPeriod(value: string) {
    localStorage.setItem("longBreakPeriod", JSON.stringify(value))
    context.setLongBreakPeriod(value)
  }

  return (
    <div className="w-full h-full flex gap-1">
      <div className="flex-1 flex flex-col gap-1 items-center justify-center">
        <span className="capitalize text-xl">Long Break</span>
